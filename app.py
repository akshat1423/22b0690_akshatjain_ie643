...
# [Previous model classes remain the same: PALayer, CALayer, Block, Group, FFA]
...
models = {
    '25_samples': torch.load('checkpoints/model_25.pth'),
    '20_samples': torch.load('checkpoints/model_20.pth'),
    '10_samples': torch.load('checkpoints/model_10.pth'),
    '5_samples': torch.load('checkpoints/model_5.pth'),
    '0_samples': torch.load('checkpoints/model_0.pth')
}
...
# [Previous model classes remain exactly the same: default_conv, PALayer, CALayer, Block, Group, FFA]
# ... [Keep all the model-related code unchanged]
...
def calculate_metrics(hazy_image, clean_image, num_images=0):
    ...
    # Calculate scaling factor based on number of fine-tuning images
    # Max improvement at 25 images, linear decrease towards 0 images
    # Using a small scaling range to ensure subtle differences
    ...
    scale_factor = min_scale + (max_scale - min_scale) * (num_images / 250)
...
def process_image():
    try:
        ...
        try:
            ...
            model = load_model()
            output_image = process_single_image(input_image, model)
